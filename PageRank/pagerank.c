...
int main(int argc, char* argv[]) {
	...
	//使用CSR压缩方式
	double *val = (double*)calloc(edges_num, sizeof(double));
	int *col_ind = (int*)calloc(edges_num, sizeof(int));
	int *row_ptr = (int*)calloc(node_num, sizeof(int));
	row_ptr[0] = 0;
	int fwomnode, tonode;
	int cur_row = 0;
	int i = 0;
	...
	int curel = 0;
	char url[300];
	char **index = (char **)malloc(sizeof *index * node_num);
	int m;
